...
import type { AboutData, AboutInput } from "@/features/about/types/AboutData";
...
class AboutDAO {
  ...
  async getAll(): Promise<AboutData[]> {
    try {
      ...
      console.error(
        "Error fetching all about documents:",
  ...
  async getById(id: string): Promise<AboutData | null> {
    try {
      ...
      console.error(
        `Error fetching about with id ${id}:`,
        error.response || error
      );
  ...
  async update(id: string, data: AboutInput): Promise<AboutData> {
    try {
      ...
      console.error(
        `Error updating about document with id ${id}:`,
        error.response || error
      );
      throw new Error(error.response?.data?.message || error.message);
    }
  ...
  async delete(id: string): Promise<boolean> {
    try {
      ...
      console.error(
        `Error deleting about document with id ${id}:`,
        error.response || error
      );
      throw error;
    }
